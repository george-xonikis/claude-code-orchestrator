 ...
 *
 * The static wording lives in DEFAULT_SYNTHESIS_TEMPLATE (admin-overridable via
 * Settings → Prompts); the builder pre-renders the dynamic pieces (shaping
 * constraint lines, ad-hoc direction, planning memory, exclusions, the two
 * reports) into string vars.
 */

import { renderTemplate } from '@/server/core/render';

/** Config-driven shaping applied to a planning pass's output. */
...
export const DEFAULT_SYNTHESIS_TEMPLATE = `You are the synthesis step of a planning meeting between a Principal
...
- Keep at most {{maxProposals}} items, ranked by leverage.
{{shapingConstraints}}
{{#adHocDirection}}
- The developer's ad-hoc planning direction below governs this pass: rank what
  they asked for first, and drop items they directed away from.
...
export function synthesisPrompt(
  ...
  return renderTemplate(template ?? DEFAULT_SYNTHESIS_TEMPLATE, {
    maxProposals: String(shaping.maxProposals),
    shapingConstraints: shapingConstraints(shaping),
    adHocDirection: adHocDirection.trim(),
