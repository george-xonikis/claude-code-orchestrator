...
/** GET /api/planning?repo=<id> -> { passes (newest first), intervalHours, productMapRun }. */
export async function GET(request: Request) {
  ...
  try {
    ...
    const { passes, intervalHours, productMapRun } = await getPlanning(repo);
    return NextResponse.json({ passes, intervalHours, productMapRun: productMapRun ?? null });
  } catch (err) {
