...
import { readSettings, writeSettings } from '@/server/knowledge/settings';

/** GET /api/settings?repo=<id> -> { goal, planningMemory }. */
...
/** POST /api/settings?repo=<id> {goal?, planningMemory?} -> save the provided fields. */
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!body || (body.goal === undefined && body.planningMemory === undefined)) {
      return badRequest('Provide goal and/or planningMemory');
    }
    for (const key of ['goal', 'planningMemory'] as const) {
